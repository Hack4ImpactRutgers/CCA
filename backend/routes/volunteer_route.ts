...
router.get('/:id', (req: Request, res: Response) => {
    Volunteer.findById(req.params.id)
        ...
        .catch((err: any) => {
            ...
            console.error(err);
            res.status(500).send({
                error: 'An error occurred fetching the volunteer.',
            });
        });
});
...
router.post(
    ...
    async (req: Request, res: Response) => {
        try {
            const pendingVolunteer = await PendingVolunteer.findById(
                req.params.id
            );
            ...
            if (!pendingVolunteer) {
                return res
                    .status(404)
                    .send({ error: 'Invalid verification request.' });
            }
            ...
            console.error(err);
            res.status(500).send({
                error: 'An error occured while verifying.',
            });
        }
    }
);
