...
router.post('/otp/request-otp', async (req, res) => {
    ...
    if (existingOTP) {
        return res
            .status(429)
            .json('An OTP is already active, please wait for it to expire.');
    }
...
router.post('/admin/forgot-password', async (req, res) => {
    ...
    if (!user) {
        return res
            .status(200)
            .json(
                'If a user with this email exists, an email will be sent to them.'
            );
    }
...
router.post('/admin/verify-forgot-password', async (req, res) => {
    ...
    const decoded = jwt.verify(token, TOKEN_SECRET as Secret) as {
        email: string;
        resetToken: string;
    };
    ...
    if (!request) {
        return res
            .status(400)
            .json('This password change request has expired or is invalid');
    }
