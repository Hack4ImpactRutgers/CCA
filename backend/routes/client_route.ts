...
router.get('/all', (req: Request, res: Response) => {
    Client.find()
        ...
        .catch((err: any) => {
            console.error(err);
            res.status(500).send({
                error: 'An error occurred fetching the clients.',
            });
        });
});
...
router.get('/:id', (req: Request, res: Response) => {
    Client.findById(req.params.id)
        ...
        .catch((err: any) => {
            ...
            console.error(err);
            res.status(500).send({
                error: 'An error occurred fetching the client.',
            });
        });
});
...
router.patch('/:id', (req: Request, res: Response) => {
    ...
    Client.findByIdAndUpdate(id, newValues)
        .then((client: any) => {
            if (!client) {
                return res.status(404).send({ error: 'Client not found.' });
            }
        ...
        .catch((err: any) => {
            console.error(err);
            res.status(500).send({
                error: 'An error occurred updating the client.',
            });
        });
});
